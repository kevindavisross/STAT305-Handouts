...
-   If a sample is selected at random, then the sample mean is a random variable that has a distribution. 
...
Let $T_n$ by the total dollar amount spent by the $n$ customers in the sample, and let $\bar{X_n}=T_n/n$ represent the sample mean dollar amount spent by the $n$ customers in the sample.
...
\
\
1.  Randomly select two customers, independently, and let $X_1$ and $X_2$ denote the amounts spent by the two people selected. 
Make a table of all possible $(X_1, X_2)$ pairs and their probabilities.
\
\
\
\
\
1.  Use the table from the previous part to find the distributions of $T_2$ and $\bar{X}_2$. 
Interpret in words in context what these distributions represent.
\
\
\
\
...
\
\
1.  Compute $\text{Var}(\bar{T}_2)$ and $\text{Var}(\bar{X}_2)$. 
How do these values relate to the population variance $\sigma^2$?
\
\
\
\
\
1.  Compute $\text{SD}(\bar{T}_2)$ and $\text{SD}(\bar{X}_2)$. 
How do these values relate to the population standard deviation $\sigma$?
\
\
\
\
...
-   The **sample mean** is
    $$
    \bar{X}_n = \frac{X_1 + \cdots + X_n}{n} = \frac{T_n}{n},
    $$
...
\
\
1.  Simulation shows that $\bar{X}_{30}$ has an approximate Normal distribution. 
Use this Normal distribution to approximate $\text{P}(\bar{X}_{30} > 6)$, and interpret the probability.
\
\
\
\
...
-   The **(Standard) Central Limit Theorem**. Let $X_1,X_2,\ldots$ be independent and identically distributed (i.i.d.) random variables with finite mean $\mu$ and finite standard deviation $\sigma$. Then
    ...
    $$
    $$
    \text{$T_n$ has an approximate $N(n\mu,\sigma\sqrt{n})$ distribution, if $n$ is large enough}
    $$
