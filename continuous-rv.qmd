...
Suppose an earthquake just occurred and let $X$ be the amount of time until the next earthquake; we'll assume @tbl-quake-percentile-table represents (partially) the distribution of $X$.
...
Evaluate and interpret $F(12.6)$ and $F(26.8)$.
\
\
\
\
...
\
\
1.  Let $Q(0.1)$ represent the value of $x$ for which $F(x)=0.1$. 
Evaluate and interpret $Q(0.1)$.
\
\
\
\
...
\
\
1.  Suppose you simulate many values of $X$. 
Sketch a histogram of $X$.
\
\
\
\
...
We can just spin the Uniform(0, 1) spinner---which would tell us what percentile that we want, e.g., 0.7 for the 70th percentile---and transform each resulting value by plugging it into the quantile function to obtain the percentile in the measurement units of the the variable.
